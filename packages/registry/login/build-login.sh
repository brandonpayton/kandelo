...
SOURCE="$KANDELO_PACKAGE_SOURCE_ROOT/programs/login.c"
OUTPUT="$KANDELO_PACKAGE_WORK_DIR/login.wasm"
if [ ! -f "$SOURCE" ] || [ -L "$SOURCE" ]; then
    echo "ERROR: login source must be a regular file: $SOURCE" >&2
    exit 1
fi
...
install_local_binary login "$OUTPUT" login.wasm
